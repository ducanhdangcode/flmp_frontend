                ...
                {player?.nationalTeamStats.map((stat) => {
                    return (
                        <tr className = "h-[2rem] bg-easeWhite text-center border-solid border-b-[1px] border-b-gray-500 font-space-grotesk">
                            <td className = "border-solid border-r-[1px] border-r-gray-500 h-[2rem] text-center py-[1rem]">
                                <img src = {stat?.leagueLogoLink} alt = "" className = "w-[1.5rem] h-[1.5rem] absolute left-0" />
                                <p>{stat?.leagueName}</p>
                            </td>
                            <td className = "border-solid border-r-[1px] border-r-gray-500 h-[2rem] text-center py-[1rem]">{stat?.appearances ? stat?.appearances : '-'}</td>
                            <td className = "border-solid border-r-[1px] border-r-gray-500 h-[2rem] text-center py-[1rem]">{player?.position === 'Goalkeeper' ? (stat?.cleanSheet ? stat?.cleanSheet : '-') : (stat?.goal ? stat?.goal : '-')}</td>
                            <td className = "border-solid border-r-[1px] border-r-gray-500 h-[2rem] text-center py-[1rem]">{player?.position === 'Goalkeeper' ? (stat?.goalConceded ? stat?.goalConceded : '-') : (stat?.assist ? stat?.assist : '-')}</td>
                            <td className = "border-solid border-r-[1px] border-r-gray-500 h-[2rem] text-center py-[1rem]">{stat?.yellowCard > 0 ? stat?.yellowCard : "-"}</td>
                            <td className = "border-solid border-r-[1px] border-r-gray-500 h-[2rem] text-center py-[1rem]">{stat?.redCard > 0 ? stat?.redCard : "-"}</td>
                            <td className = "border-solid border-r-[1px] border-r-gray-500 h-[2rem] text-center py-[1rem]">{`${stat?.minutePlayed ? stat?.minutePlayed : '-'}'`}</td>
                        </tr>
                    )
                })}
